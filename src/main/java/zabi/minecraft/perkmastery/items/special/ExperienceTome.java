		...
		for (int sx = -radius; sx <= radius; sx++)
			for (int sy = -radius; sy <= radius; sy++)
				for (int sz = -radius; sz <= radius; sz++) {
					if (world.getBlock((int) player.posX + sx, (int) player.posY + sy, (int) player.posZ + sz).equals(Blocks.bookshelf)) listaBlocchi.add(new ChunkCoordinates((int) player.posX + sx, (int) player.posY + sy, (int) player.posZ + sz));
