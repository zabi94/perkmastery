...
public class DigHandler {
	...
	public static void applyFortune(BreakEvent evt) {
		if (!evt.block.getItemDropped(evt.blockMetadata, evt.world.rand,0).equals(Item.getItemFromBlock(evt.block)))
		evt.world.spawnEntityInWorld(new EntityItem(evt.world, evt.x, evt.y, evt.z, new ItemStack(evt.block.getItemDropped(evt.blockMetadata, evt.world.rand,0),1,evt.block.damageDropped(evt.blockMetadata))));
	}
...
//			if (evt.world.isRemote) PerkMastery.proxy.getSinglePlayer().addChatMessage(new ChatComponentText(StatCollector.translateToLocal("general.recursiveLimit")));
			return;
		}
	...
	public static void applyCrumbling(int x, int y, int z, World world, int qtDestroyed) {
		if (qtDestroyed>Config.maxIterations) {
//			if (world.isRemote) PerkMastery.proxy.getSinglePlayer().addChatMessage(new ChatComponentText(StatCollector.translateToLocal("general.recursiveLimit")));
			return;
		}
