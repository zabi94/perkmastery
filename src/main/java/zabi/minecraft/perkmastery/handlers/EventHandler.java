...
//				Log.i("On join server:");
//				NBTTagCompound tg=(NBTTagCompound) player.getEntityData().getTag(ExtendedPlayer.TAG_PREFIX);
//				Log.i(tg.toString());
				
			} else {
				player=PerkMastery.proxy.getSinglePlayer();
//				Log.i("On join client:");
//				NBTTagCompound tg=(NBTTagCompound) player.getEntityData().getTag(ExtendedPlayer.TAG_PREFIX);
//				Log.i(tg.toString());
			}
			...
			//Fa un toggle dell'attività per sincronizzare il HackyPlayerControllerMP
...
//			if (FMLCommonHandler.instance().getEffectiveSide()==Side.CLIENT && PerkMastery.proxy.getSinglePlayer().getDisplayName().equals(player.getDisplayName())) {
//				if (ExtendedPlayer.isEnabled(PerkMastery.proxy.getSinglePlayer(), PlayerClass.BUILDER, 4))
//					PerkMastery.proxy.getSinglePlayer().fallDistance = PerkMastery.proxy.getSinglePlayer().fallDistance/2F;
//			}
	...
	public void onLivingHurt(LivingHurtEvent event) {
		...
		if (event.entityLiving instanceof EntityPlayer) {
			...
			if (ExtendedPlayer.isEnabled(player, PlayerClass.WARRIOR, 3)) {
				for (int i=0;i<4;i++) if (ExtendedPlayer.getExtraInventory(player, InventoryType.REAL)[19+i]!=null) {
	...
	public void onPlayerTrySleep(PlayerSleepInBedEvent evt) {
		if (ExtendedPlayer.isEnabled(evt.entityPlayer, PlayerClass.EXPLORER, 1) && !evt.entityPlayer.worldObj.isDaytime()) {
	        ...
	        else evt.entityPlayer.setPosition((double)((float)evt.x + 0.5F), (double)((float)evt.y + 0.9375F), (double)((float)evt.z + 0.5F));
...
//		if (event.entityPlayer.worldObj.isRemote && PerkMastery.proxy.getSinglePlayer().getDisplayName().equals(event.entityPlayer.getDisplayName())) {
//			ExtendedPlayer.restoreTag(PerkMastery.proxy.getSinglePlayer(), ExtendedPlayer.backupTag(event.original));
//		}
	}
    ...
    public void onRenderWorldLastEvent(RenderWorldLastEvent event) {
        ...
        try {
        	while (it.hasNext()) it.next().render(event.partialTicks);
        } catch (Exception e) {
